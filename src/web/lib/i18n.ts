...
const EN = {
  ...
  answerNoOptions:
    'The choices are drawn by the terminal and are not written down, so they cannot be labelled here. The keys below move and confirm.',
  answerKeysLabel: 'Answer keys',
  answerKeysFallback: 'Or use the terminal keys if this does not match what Attach shows.',
...
const ZH: Record<Key, string> = {
  ...
  answerNoOptions: '这些选项由终端绘制、并未写入记录，因此无法在此列出。下方按键用于移动和确认。',
  answerKeysLabel: '回复按键',
  answerKeysFallback: '如果与「接入」标签页显示的不一致，可改用终端按键。',
  answerPlan: '待审阅的计划',
