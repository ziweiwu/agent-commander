...
`src/shared/types.ts` (424 lines) and `rust/src/types.rs` (597 lines, 94 serde
attributes) describe one wire format. `AGENTS.md` says outright that they are
"edited together or not at all, because nothing checks that they still agree".

Derive the TypeScript from the Rust with `ts-rs` or `typeshare`, and this stops
being a property anyone has to hold. That is strictly better than testing it:
the golden fixtures in `rust/tests/golden/` and the 263 e2e tests currently stand
in for a compiler, and they only catch a drift that happens to change a captured
response.

**Watch for:** `src/shared/types.ts` is not only types. It also carries
`ALLOWED_KEYS`, `DESTRUCTIVE_KEYS`, `MODEL_ALIASES`, `MODE_CYCLE` and
`SPAWN_MODES` — value exports the browser imports directly, which is what makes
"the server validates against them and the browser offers exactly them" true.
Codegen has to emit those as values, not just as types, or that guarantee is
lost. Decide that before starting; it may mean a generated module plus a
hand-written one that re-exports.

**Done when:** the TypeScript is generated by a script in `package.json`, a
stale checkout fails a gate rather than shipping, and `npm test` (including the
golden fixtures) and `npm run e2e` pass unchanged.
