...
ICONS: dict[str, str] = {
    ...
    # A picture to hand an agent. The frame is `screen`'s rectangle squared up,
    # because that shape is the one already proven to survive 11px here; inside
    # it the single diagonal ridge and one dot are the most that stays legible
    # at that size. A paperclip was drawn first and lost its bend entirely.
    "picture": '<rect x="3.6" y="4.6" width="16.8" height="14.8" rx="2.4"/><path d="M4.4 16.4L9.6 10.6L14.4 15.4"/><circle cx="15.6" cy="9.2" r="1.2"/>',
}
