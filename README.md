...
npx @ziweiwu/agent-commander --install-app   # macOS: put an icon in ~/Applications
```

The Mac app is a **launcher**, not a self-contained application. It finds the
Node you already have (20 or newer), starts the same server on the same port,
...
```
pkill -f 'Agent Commander.app.*dist/server/cli.js'
```
...
It is built on your machine and unsigned, so it runs there without ceremony —
locally created files are never quarantined — but copying it to another Mac will
trip Gatekeeper. That is a consequence of not signing it, not a fault. Building
it needs the Xcode Command Line Tools, which supply both `iconutil` and the
`python3` the builder runs under (`xcode-select --install`). Its log, including
which Node and which `PATH` it found, is at
`~/Library/Logs/agent-commander/server.log`.

**Updating it** is the same command again:

```
npx @ziweiwu/agent-commander@latest --install-app     # or, from a clone:
npm run app:install
```

The app carries its own copy of the code, so the running server keeps serving
what it started with until it is replaced. The launcher notices — if the bundle
is newer than the server it started, it stops that one and starts the new one
rather than opening a dashboard that is quietly out of date. It will only ever
replace a server it started itself: a copy you ran from a terminal is left
alone, and opened.

From a clone, `npm run app` builds into `build/mac/` without installing.
...
-p, --port <n>      port to listen on (default 4317)
    ...
    --install-app   macOS: build the launcher app into ~/Applications and exit
    --install-statusline
...
npm test           # 800 tests: pure logic, server, and React components
