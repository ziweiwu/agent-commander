...
all, so today the rule is pinned at the server (`test/subagents.test.ts` keeps
`bytes` as bytes) and at the tree (`test/ui/fleet-delegates.test.tsx` asserts
nothing size-shaped reaches the screen); any surface that ever draws it again
inherits the caption clause.
