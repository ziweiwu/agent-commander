...
impl AgentPatch {
    ...
    pub fn apply(&self, a: &mut Agent) {
        if let Some(v) = &self.activity { a.activity = Some(v.clone()) }
        if let Some(v) = self.last_activity_at { a.last_activity_at = Some(v) }
        if let Some(v) = self.tokens { a.tokens = Some(v) }
        if let Some(v) = self.subagents { a.subagents = Some(v) }
        if let Some(v) = self.delegating { a.delegating = Some(v) }
        if let Some(v) = &self.ai_title { a.ai_title = Some(v.clone()) }
        if let Some(v) = &self.last_prompt { a.last_prompt = Some(v.clone()) }
        if let Some(v) = &self.permission_mode { a.permission_mode = Some(v.clone()) }
        if let Some(v) = &self.model { a.model = Some(v.clone()) }
        if let Some(v) = &self.goal { a.goal = v.clone() }
        if let Some(v) = self.status { a.status = v }
        if let Some(v) = &self.waiting_for { a.waiting_for = v.clone() }
        if let Some(v) = &self.git_branch { a.git_branch = Some(v.clone()) }
        if let Some(v) = &self.cwd {
            a.cwd = v.clone();
            // Keep the precomputed basename in step with the path it describes.
            a.folder = std::path::Path::new(v)
                .file_name()
