...
/// How much of a user-supplied name survives into the generated session name.
///
/// Long enough to tell two sessions apart at a glance in `tmux ls`, short
/// enough that the name stays a name: everything after this is decoration on
/// something the timestamp has already made unique.
const MAX_SUFFIX_CHARS: usize = 24;
...
pub async fn validate_dir(input: &str, home: &Path) -> Result<PathBuf, SpawnError> {
    ...
    let entry = tokio::fs::metadata(&dir)
        .await
...
mod tests {
    ...
    /// Any epoch millisecond will do; the tests are about what happens to the
    /// name beside it, never about the clock.
    const SPAWNED_AT: i64 = 7;

    /// `new-session -d -s <name> -c <dir> claude` — the seven entries every
    /// spawn carries, before an optional flag is appended to them.
    const BASE_ARGV_LEN: usize = 7;
    ...
    fn bounds_the_length() {
        // Comfortably past the cap, so this is a test about truncation rather
        // than about the length of some plausible name.
        const OVERLONG: usize = 80;

        assert_eq!(
            session_name(1000, &"x".repeat(OVERLONG)),
            format!("claude-1000-{}", "x".repeat(MAX_SUFFIX_CHARS))
        );
    }
    ...
    fn inv7_a_generated_name_is_always_safe_to_hand_tmux() {
        for hostile in ["a:b", "%1", "@2", "a.b", "$(x)", "`x`", "'", "\"", "\n", "\0", "..", "-"] {
            let name = session_name(SPAWNED_AT, hostile);
            assert!(
                name.chars().all(|c| c.is_ascii_alphanumeric() || c == '-'),
                "{hostile:?} produced {name}"
            );
            assert!(name.starts_with(&format!("{SESSION_PREFIX}-{SPAWNED_AT}")), "{name}");
        }
    }
    ...
    fn a_blank_name_or_model_adds_no_flag() {
        ...
        let argv = spawn_argv("claude-1", Path::new("/w"), &r, "claude");
        assert_eq!(argv.len(), BASE_ARGV_LEN, "{argv:?}");
    }
