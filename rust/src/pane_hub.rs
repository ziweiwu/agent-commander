...
fn unchanged(previous: &Sample, latest: &Sample) -> bool {
    previous.meta.cursor_x == latest.meta.cursor_x
        && previous.meta.cursor_y == latest.meta.cursor_y
        && previous.meta.rows == latest.meta.rows
        && previous.meta.cols == latest.meta.cols
        && previous.lines == latest.lines
}

/// Why the loop is being hurried along.
///
/// The two callers want different things from it. A write is a promise that
/// the pane is about to change, so for a moment afterwards an unchanged read
/// means "not yet" rather than "quiet", and must not slow the loop down. A tab
/// merely attaching promises nothing, so it gets the fast read it needs and
/// then lets the pane's own behaviour decide the cadence.
#[derive(Clone, Copy, PartialEq, Eq)]
enum Wake {
    /// This app has just written to the pane, so a redraw is owed.
    ExpectingEcho,
    /// A tab has just attached and has nothing on screen yet.
    Attaching,
}
...
impl PaneLoop {
    ...
    /// happen.
    ///
    /// [`Wake`] separates those two, and only one of them holds the loop at
    /// full speed through reads that find nothing.
    fn wake(&self, reason: Wake) {
        {
            ...
            if reason == Wake::ExpectingEcho {
                st.hot_until = Instant::now() + Duration::from_millis(HOT_MS);
            }
            // No chain to hurry along. The TS returns here for the same reason
            // — there is no timer to clear — and it matters because a `Notify`
            // *stores* its permit: left to accumulate, the wake that `subscribe`
            // issues before `start` would be consumed after the chain's first
            // read and buy a free back-to-back second one, which is exactly the
            // duty cycle INV-4 says a poll may not exceed.
            if !st.active {
                return;
            }
        }
    ...
    async fn run(self: Arc<Self>) {
        loop {
            ...
            let Some(delay) = self.next_delay(started) else {
                return;
            };
            tokio::select! {
    ...
    /// Whether the chain should end here, marking it over on the way out so
    /// that a later `start` may begin a new one.
    ...
    /// Fold one read into the loop's state, and say what subscribers are told.
    ///
    /// This is where the cadence is decided, because this is the only place
    /// that knows whether anything moved.
    fn record(&self, read: anyhow::Result<Sample>) -> HubEvent {
        let sample = match read {
            Ok(sample) => Arc::new(sample),
            // The loop keeps running. Whether a failure is fatal is the
            // subscriber's call, not this loop's: a pane that has exited is
            // over, but a tmux that could not be reached for one tick is not,
            // and treating those the same is what used to stop a terminal for
            // good over a transient EAGAIN.
            Err(err) => return HubEvent::Error(Arc::new(err)),
        };
        let mut st = self.state.lock().unwrap();
        ...
        if changed {
            ...
            // Unchanged, and nothing has been typed recently enough to expect
            // a redraw. Only now is "quiet" the right conclusion.
            st.interval_ms = MAX_MS.min((st.interval_ms as f64 * IDLE_GROWTH).round() as u64);
        }
        st.last = Some(sample.clone());
        HubEvent::Sample(sample)
    }

    fn deliver(&self, event: &HubEvent) {
        // Delivered from a snapshot because a listener cannot be called while
        // the lock it might unsubscribe through is held — but each one is
        // re-checked first, so a listener that tears down a *different* one
        // mid-delivery (the tab that answers a dead pane by releasing its
        // neighbour) still stops that one being called. That is what iterating
        // a live `Set` gives the TS.
        ...
        for (id, listener) in snapshot {
            ...
            // `panic = "abort"` (the release profile) nothing can catch that —
            // so this holds in dev and test, and listeners are expected not to
            // panic in the first place.
            let _ = std::panic::catch_unwind(AssertUnwindSafe(|| listener(event)));
        }
    }

    /// How long to wait before the next read, or `None` when the chain ends
    /// here because nobody is watching any more.
    fn next_delay(&self, started: Instant) -> Option<Duration> {
        let mut st = self.state.lock().unwrap();
        ...
        // Never start the next read sooner than the last one took. Under load
        // that is what stops the app adding to the congestion it is already
        // waiting on. A wake that arrived mid-read is the one exception, and it
        // is handled by the stored `Notify` permit: the pane has just been
        // written to, the user is watching for the echo, and this is a single
        // catch-up read rather than a faster steady state.
        ...
        Some(Duration::from_millis(interval.max(elapsed)))
    }
}
...
impl PaneHub {
    ...
    pub fn subscribe(
        ...
        let loop_ = self.loop_for(pane_id);
        loop_.state.lock().unwrap().listeners.push((id, listener.clone()));
        loop_.wake(Wake::Attaching);
        ...
        loop_.start();
        self.unsubscriber(pane_id, id)
    }

    /// The loop for this pane, created if this is the first tab to ask for it.
    fn loop_for(&self, pane_id: &str) -> Arc<PaneLoop> {
    ...
    /// The handle that drops one listener — and the pane's loop with it, once
    /// the last watcher has gone.
    fn unsubscriber(&self, pane_id: &str, id: u64) -> Unsubscribe {
...
mod tests {
    ...
    /* The fixture timings these tests are written against. Each is a duration
       in milliseconds, named for the part it plays rather than for its value:
       the loop's own cadence is `BASE_MS` and `MAX_MS` above, and everything
       here is chosen relative to those. */

    /// A read slow enough that a second subscriber reliably arrives while one
    /// is still in flight — the case that used to start a second tick chain.
    const READ_SPANNING_A_SUBSCRIBE_MS: u64 = 20;
    /// A read slow enough that a frame arriving sooner than this cannot have
    /// come from a read of its own, and so must have come from the cache.
    const READ_SLOWER_THAN_A_CACHED_FRAME_MS: u64 = 80;
    /// A read that takes most of an interval, so reads and sleeps overlap the
    /// way they do on a busy machine.
    const READ_NEARLY_A_WHOLE_INTERVAL_MS: u64 = 60;
    /// A read short enough to leave room for several inside a hot window.
    const SHORT_READ_MS: u64 = 30;

    /// Long enough for a handful of reads at the steady cadence.
    const A_FEW_INTERVALS_MS: u64 = BASE_MS * 5 / 2;
    /// Long enough for one read, and not for two.
    const AN_INTERVAL_AND_A_HALF_MS: u64 = BASE_MS * 3 / 2;
    /// Long enough that a loop still polling would prove it by now.
    const THREE_INTERVALS_MS: u64 = BASE_MS * 3;
    /// Four intervals: enough to measure a steady rate rather than a burst.
    const FOUR_INTERVALS_MS: u64 = BASE_MS * 4;
    /// Long enough for the backoff to have reached its ceiling.
    const FULLY_BACKED_OFF_MS: u64 = MAX_MS * 3;
    /// Past the ceiling, so the loop is as slow as it is ever allowed to get.
    const PAST_THE_BACKOFF_MS: u64 = MAX_MS * 3 / 2;

    /// Enough time for a quick read to have happened, and little else.
    const ONE_QUICK_READ_MS: u64 = 20;
    /// Enough for the first read of a freshly attached loop to complete.
    const ENOUGH_FOR_THE_FIRST_READ_MS: u64 = 30;
    /// Past the end of a `READ_SLOWER_THAN_A_CACHED_FRAME_MS` read and short of
    /// the one after it, so exactly one frame exists to be cached.
    const ENOUGH_FOR_ONE_SLOW_READ_MS: u64 = 120;
    /// Long enough for many reads, whatever the cadence settles at.
    const A_RUN_OF_MANY_READS_MS: u64 = 600;
    /// Long enough for several slow reads back to back.
    const SEVERAL_SLOW_READS_MS: u64 = 500;
    /// A window in which a pane redrawing nothing must have been read fewer
    /// times than the steady cadence would allow.
    const IDLE_WINDOW_MS: u64 = 700;
    /// Long enough after a wake for the caught-up reads to show up.
    const AFTER_A_WAKE_MS: u64 = 400;
    /// Halfway through a `READ_NEARLY_A_WHOLE_INTERVAL_MS` read.
    const MID_SLOW_READ_MS: u64 = 30;
    /// Halfway through a `SHORT_READ_MS` read, so one is still in flight.
    const MID_SHORT_READ_MS: u64 = 15;
    /// Less than one steady interval, so a read inside it cannot have been
    /// paced by one.
    const LESS_THAN_ONE_INTERVAL_MS: u64 = 110;
    /// Long enough for the loop to settle into whatever cadence it has chosen.
    const SETTLING_MS: u64 = 50;
    /// Long enough for the read that carries an echo to have happened.
    const ENOUGH_FOR_THE_ECHO_MS: u64 = 60;

    /// At the steady cadence one interval allows about one read; three is the
    /// most that can be explained by scheduling jitter rather than by hurrying.
    const STEADY_CADENCE_READS_CEILING: usize = 3;
    /// Reads at 30ms with a `BASE_MS` floor allow about four in four
    /// intervals. Back-to-back it would be a dozen or more.
    const FOUR_INTERVALS_READS_CEILING: usize = 6;
    /// Three tabs on one agent: the case that used to cost three times the
    /// tmux traffic of one while the hub still reported a single loop.
    const SUBSCRIBERS_SHARING_ONE_PANE: usize = 3;
    ...
    impl FakePanes {
        ...
        fn scripted(
            script: impl Fn(usize) -> Result<Sample, String> + Send + Sync + 'static,
        ) -> Arc<FakePanes> {
            FakePanes::build(0, script)
        }
        fn build(
            delay_ms: u64,
            script: impl Fn(usize) -> Result<Sample, String> + Send + Sync + 'static,
    ...
    async fn costs_the_same_tmux_traffic_however_many_subscribers() {
        async fn reads_for(subscribers: usize) -> usize {
            let panes = FakePanes::slow(READ_SPANNING_A_SUBSCRIBE_MS);
            let hub = PaneHub::new(Arc::new(panes.clone()));
            let mut offs: Vec<Unsubscribe> = Vec::new();
            for _ in 0..subscribers {
                offs.push(hub.subscribe("%1", Box::new(|_: &HubEvent| {})));
            }
            tick(A_RUN_OF_MANY_READS_MS).await;
            for off in offs {
                off();
            }
            panes.reads()
        ...
        let many = reads_for(SUBSCRIBERS_SHARING_ONE_PANE).await;
        assert!(one > 1, "{one}");
        ...
        // each.
        assert!(many <= one + 2, "one={one} many={many}");
    }
    ...
    async fn gives_a_tab_that_joins_late_the_last_read_at_once() {
        ...
        // quick.
        let panes = FakePanes::slow(READ_SLOWER_THAN_A_CACHED_FRAME_MS);
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let first = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        tick(ENOUGH_FOR_ONE_SLOW_READ_MS).await;
    ...
    async fn stops_polling_once_the_last_tab_leaves() {
        ...
        tick(AN_INTERVAL_AND_A_HALF_MS).await;
        let at_stop = panes.reads();
        off();
        tick(THREE_INTERVALS_MS).await;
    ...
    async fn never_starts_a_read_before_the_previous_one_finished() {
        let panes = FakePanes::slow(READ_NEARLY_A_WHOLE_INTERVAL_MS);
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        tick(SEVERAL_SLOW_READS_MS).await;
        off();
    ...
    async fn backs_off_while_unchanged_and_snaps_back_when_it_moves() {
        ...
        assert!(idle_reads < (IDLE_WINDOW_MS / BASE_MS) as usize, "{idle_reads}");
        ...
        hub.wake("%1");
        tick(AFTER_A_WAKE_MS).await;
        assert!(panes.reads() - idle_reads > 1);
        off();
    }

    #[tokio::test]
    async fn inv4_never_backs_off_past_its_ceiling() {
        // The backoff is a saving, not a licence to stop looking: a pane that
        // has been quiet for a minute must still be read once a second, or an
        // agent that finishes overnight is reported as still thinking.
        let panes = FakePanes::scripted(|_| Ok(lines("same")));
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        tick(FULLY_BACKED_OFF_MS).await;

        let before = panes.reads();
        tick(MAX_MS * 2 + MAX_MS / 2).await;
        let while_quiet = panes.reads() - before;
        assert!(while_quiet >= 2, "the ceiling was breached: {while_quiet} reads");
        off();
    }

    /// INV-4's other half: the backoff must be *cheap*, and it is only cheap if
    /// `unchanged` is judged on what the viewer draws. `alternate` and
    /// `pane_dead` come back in the same round trip and are deliberately not
    /// part of the comparison — they are answered from the pane's own flags
    /// rather than by diffing, and folding them in would make a pane entering a
    /// full-screen TUI look like a redraw.
    #[tokio::test]
    async fn inv4_judges_quiet_on_what_the_viewer_draws() {
        let same = |meta: PaneMeta| Sample { meta, lines: vec!["a".into()] };
        let alt = PaneMeta { alternate: true, ..META };
        let dead = PaneMeta { dead: true, ..META };
        assert!(unchanged(&same(META), &same(alt)));
        assert!(unchanged(&same(META), &same(dead)));
        // What the viewer *does* draw:
        assert!(!unchanged(&same(META), &same(PaneMeta { cursor_x: 1, ..META })));
        assert!(!unchanged(&same(META), &same(PaneMeta { cursor_y: 1, ..META })));
        assert!(!unchanged(&same(META), &same(PaneMeta { cols: 81, ..META })));
        assert!(!unchanged(&same(META), &same(PaneMeta { rows: 4, ..META })));
        assert!(!unchanged(
            &same(META),
            &Sample { meta: META, lines: vec!["b".into()] }
        ));
    }
    ...
    async fn stays_at_full_speed_for_a_moment_after_a_wake() {
        ...
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        tick(PAST_THE_BACKOFF_MS).await; // let it back off properly
        hub.wake("%1");
    ...
    async fn goes_quiet_again_once_the_wake_window_has_passed() {
        ...
        assert!(
            panes.reads() - settled <= STEADY_CADENCE_READS_CEILING,
            "{}",
            panes.reads() - settled
        );
        off();
    ...
    async fn reads_immediately_when_woken_rather_than_waiting_out_the_backoff() {
        ...
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        tick(PAST_THE_BACKOFF_MS).await;
        let before = panes.reads();
    ...
    async fn a_wake_mid_read_starts_the_next_read_immediately() {
        ...
        tick(MID_SLOW_READ_MS).await; // a read is now in flight
        hub.wake("%1");
        let before = panes.reads();
        // The in-flight read finishes at ~60ms; the next must follow at once
        // rather than after BASE_MS.
        tick(LESS_THAN_ONE_INTERVAL_MS).await;
        assert!(panes.reads() > before);
        off();
    }

    #[tokio::test]
    async fn inv4_does_not_turn_one_wake_into_a_permanently_faster_loop() {
        // The catch-up read is a single one, not a new steady state. If the
        // stored wake were consumed more than once — or re-armed by the read it
        // triggers — one keystroke would leave the pane polled at the read rate
        // for as long as the tab stayed open.
        let panes = FakePanes::slow(SHORT_READ_MS);
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));

        tick(MID_SHORT_READ_MS).await; // a read is in flight
        hub.wake("%1");
        tick(HOT_MS + BASE_MS).await; // let the hot window expire

        let before = panes.reads();
        tick(FOUR_INTERVALS_MS).await;
        let steady = panes.reads() - before;
        assert!(
            steady <= FOUR_INTERVALS_READS_CEILING,
            "one wake left the loop permanently fast: {steady}"
        );
        off();
    }

    /// INV-4: a subscriber joining costs a read, not a chain.
    ///
    /// `subscribe` wakes the loop before starting it, and a stored wake permit
    /// with no chain to spend it on would be spent by the *first* read of the
    /// chain that follows — buying a free back-to-back second read that no
    /// interval paced. The TS cannot have this bug (there is no timer to
    /// cancel, so its `wake` returns); this asserts the Rust does not either.
    #[tokio::test]
    async fn inv4_a_first_subscribe_does_not_buy_a_free_extra_read() {
        let panes = FakePanes::scripted(|_| Ok(lines("same")));
        let hub = PaneHub::new(Arc::new(panes.clone()));
        let off = hub.subscribe("%1", Box::new(|_: &HubEvent| {}));
        // Well inside one interval: exactly one read may have happened.
        tick(BASE_MS / 2).await;
        assert_eq!(panes.reads(), 1, "the first read was doubled");
        off();
    }
    ...
    async fn looks_far_more_often_than_the_steady_cadence() {
        ...
        assert!(
            panes.reads() - before > STEADY_CADENCE_READS_CEILING,
            "{}",
            panes.reads() - before
        );
        off();
    ...
    async fn stops_hurrying_as_soon_as_the_redraw_arrives() {
        ...
        *content.lock().unwrap() = "after".to_string(); // the echo lands on the very next read
        tick(ENOUGH_FOR_THE_ECHO_MS).await;
        ...
        assert!(
            panes.reads() - after_echo <= STEADY_CADENCE_READS_CEILING,
            "{}",
            panes.reads() - after_echo
        );
        off();
    }
    ...
    async fn one_loop_per_pane() {
        ...
        tick(ENOUGH_FOR_THE_FIRST_READ_MS).await;
        for off in offs {
            off();
        }
        assert_eq!(hub.size(), 0);
    ...
    /// A pane API with no combined read, recording the calls it is given.
    ///
    /// `sample` is one tmux round trip and is what the real adapter
    /// implements; the pair is what the mocks and the test doubles offer, and
    /// they have no round trip to save.
