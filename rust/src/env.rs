...
mod tests {
    ...
    impl CommandRunner for FakeRunner {
        async fn run(&self, bin: &str, args: &[&str], _t: Duration) -> Option<String> {
    ...
    async fn server_env_degrades_rather_than_erroring() {
        let r = FakeRunner::new(vec![]);
        let env = server_env_with(4123, &r).await;
        assert!(env.tailscale.is_none());
        assert!(!env.tmux);
        assert_eq!(env.port, 4123);
    ...
    async fn exec_runner_returns_none_on_non_zero_exit() {
        let r = ExecRunner;
        assert!(r.run("false", &[], Duration::from_secs(5)).await.is_none());
        assert_eq!(
            r.run("echo", &["hi"], Duration::from_secs(5)).await.as_deref(),
            Some("hi\n")
        );
    }
    ...
    async fn exec_runner_gives_up_on_a_slow_child() {
        ...
        assert!(r.run("sleep", &["30"], Duration::from_millis(150)).await.is_none());
