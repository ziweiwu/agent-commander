...
describe('the bundle layout', () => {
  /*
   * Three of the four path constraints this file once guarded are gone with the
   * Node server, and their tests went with them: `dist/shared` beside
   * `dist/server`, a `package.json` declaring `"type": "module"`, and the
   * bridge script two levels up. A Rust binary carries no module graph to
   * resolve and no interpreter to mislead.
   *
   * The fourth survives, and still fails the quiet way: the server has to be
   * able to find the web assets. It is satisfied twice over — the launcher
   * passes `--web-root` outright, and the layout also puts `web` beside `bin`
   * so the binary's own `../web` fallback lands in the same place for anyone
   * running the staged binary by hand.
   */
  it('ships the binary and the web assets it serves', () => {
    const layout = plan()
    expect(layout.paths).toContain('Contents/Resources/bin/agent-commander')
    expect(layout.paths).toContain('Contents/Resources/web/index.html')
  })

  it('lays the web root out as a sibling of the binary', () => {
    const layout = plan()
    expect(join(layout.binary, '..', '..', 'web')).toBe(join(layout.webRoot))
  })

  // The launcher passes it explicitly, so that is what must agree.
  it('passes the web root the launcher actually names', () => {
    const launcher = readFileSync(LAUNCHER, 'utf8')
    expect(launcher).toMatch(/--web-root/)
    expect(launcher).toMatch(/Contents\/Resources\/web|Resources\/web|\$WEB/)
  })
